/**
 * Counter path regression — same checks as golden path, for "Build a counter app".
 */
...
export async function runCounterPathRegression(): Promise<boolean> {
  failures.length = 0;
  console.log(`\nCounter path regression: "${COUNTER_PATH_PROMPT}"\n`);

  const report = await buildFromPrompt({ prompt: COUNTER_PATH_PROMPT });
  const reportText = formatBuildReport(report);

  console.log(reportText);
  console.log('\n── Assertions ──\n');

  if (report.appType !== 'counter') {
    fail('0. app type is counter', `got "${report.appType}"`);
  } else {
    pass('0. app type is counter');
  }
